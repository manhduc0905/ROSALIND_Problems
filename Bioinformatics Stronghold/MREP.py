            ...
            if existing_char == s[i]:
                ...
                if active_node == root:
                    ...
                    if active_node.suffix_link:
                        ...
                        active_node = root
                    remainder -= 1
def count_k(node, cur_str):
    
    for char in node.children:
        node1 = node.children[char]
        if (s[node1.end[0]] == '$'):
            temp = cur_str + s[node1.start : node1.end[0]]
        else:
            temp = cur_str + s[node1.start : node1.end[0] + 1]
            ...
            if (len(temp) >= 20): 
                ...
                while ind != -1:
                    if (ind == 0):
                        ...
                        left_chars.add(s[ind -1])
                    ind = s.find(temp, ind + 1)
                if (len(left_chars) > 1):
                    print(temp)
                    f2.write(temp + "\n")
        count_k(node1,temp)
